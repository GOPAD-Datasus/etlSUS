...
MODULE_PATH = 'etlsus.loading.loading'


class TestLoad(unittest.TestCase):

    @patch('etlsus.loading.loading.insert_into_db')
    @patch('etlsus.loading.loading.pd.read_parquet',
           return_value=MagicMock())
    @patch('etlsus.loading.loading.load_files',
           return_value=["file1.parquet", "file2.parquet"])
    @patch('etlsus.loading.loading.create_db_engine')
    @patch('etlsus.loading.loading.config.PROCESSED_DIR', '/processed')
    def test_load_success(self, mock_create_engine, mock_load_files,
        ...
        mock_load_files.assert_called_with('/processed',
                                           endswith='.parquet')
    ...
    @patch(f'{MODULE_PATH}.create_db_engine')
    @patch(f'{MODULE_PATH}.load_files', return_value=[])
    ...
    @patch(f'{MODULE_PATH}.create_db_engine')
    @patch(f'{MODULE_PATH}.load_files')
    ...
    @patch(f'{MODULE_PATH}.load_files',
           return_value=['file1.parquet', 'file2.parquet'])
    @patch(f'{MODULE_PATH}.insert_into_db')
